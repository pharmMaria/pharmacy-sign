...
st.title("💊 약국 매대 안내문 자동 생성기")
st.markdown("제품 이름을 입력하면 평균 가격과 AI 추천 문구가 포함된 안내문이 완성됩니다!")

product_name = st.text_input("제품 이름을 입력하세요 (예: 텐텐, 타이레놀)", placeholder="여기에 약 이름을 적어주세요")

if st.button("✨ 안내문 생성하기"):
    if product_name:
        with st.spinner("AI가 추천 문구를 작성하고 있습니다... 잠시만 기다려주세요."):
            
            avg_price = mock_price_db.get(product_name, "가격 정보 없음 (데이터베이스에 없음)")
